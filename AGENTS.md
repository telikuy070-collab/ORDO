...
### Обязательное условие для владельца
Перед тем как агент начинает реализовывать задачу, владелец должен:
1. Зафиксировать решение в ADR.
2. Зафиксировать контракт и границы в RFC.
3. Создать task с приёмкой и зависимостями.
4. Передать агенту один конкретный промпт с ссылкой на этот task.

### Обязательное условие для агента
Агент:
- работает только по указанному task;
- не добавляет новые сущности, модули и зависимости вне RFC;
- не меняет стек, правила и структуру без явного указания владельца;
- сообщает о рисках и несоответствиях до выполнения изменений;
- не берет на себя решения, которые относятся к продуктовой политике или архитектуре.

### Правило согласования
Если задача требует изменения архитектуры, структуры репозитория, стека, RBAC, моделей данных или контрактов, это считается изменением продукта и должно быть подтверждено владельцем в ADR/RFC до реализации.

### Правило зоны ответственности
- Владелец принимает бизнес-решения и согласует архитектуру.
- Агент реализует только то, что описано в task и согласовано в ADR/RFC.
- Если в задаче нет достаточной информации, агент обязан остановиться и попросить уточнение, а не домысливать за владельца.

### Проверка перед завершением
Перед закрытием задачи агент обязан:
- сверить реализацию с task и RFC;
- проверить, что изменения не выходят за scope;
- прогнать обязательные проверки (`pnpm typecheck`, `pnpm lint`, `pnpm test`);
- зафиксировать в итоговом ответе, что именно было сделано и какие проверки прошли.
